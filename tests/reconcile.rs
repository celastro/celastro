...
//! cluster whose nodes disagree forever. Random histories of creations and
//! drops on several nodes, reconciled pairwise in random orders until nothing
//! changes, must end with every node holding the same definitions -- and
//! those must be what the history's last word on each name says.
...
fn random_histories_on_three_nodes_converge_to_the_last_word_on_each_name() {
    ...
    for seed in 1..=seeds {
        ...
        let dirs: Vec<PathBuf> = (0..3).map(|i| dir(&format!("{seed}-{i}"))).collect();
