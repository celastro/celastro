...
- **The zero-dependency rule is absolute.** Nothing outside `std`, no crates, no
  C libraries. A patch that adds a dependency cannot be taken no matter what it
  does, and that is easier to state up front than to litigate per PR.
