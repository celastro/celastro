...
//! (`celastro-0.celastro_9000`), or `local` for a node with no address, so
...
/// The key component a node's address becomes: `tcp://celastro-0.celastro:9000`
/// is `celastro-0.celastro_9000`, and no address is `local`.
