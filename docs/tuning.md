...
| `CELASTRO_SEAL_IDENTITY` | `2` (this build's) | The identity and framing encrypted files are written under, `1` pinned to what 0.83.0 and earlier read: the shard and file alone in the seal identity (no collection), logs without a header and an id of their own, a `CELK2` key ring. Reads are always both. Set it through the first days on 0.84.1 or later so a rollback can still open the directory; lift it once the release is trusted, and `celastro key rotate` re-seals every file under the current identity. Costs what the current identity guards against: a file of one collection standing in for another's, a log's record for another log's, a file cut at a frame boundary opening shorter. Nothing for a database in the clear. |
