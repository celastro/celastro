...
| `CELASTRO_REPLICATION` | `sync` | Whether a write is acknowledged once every live follower has it on disk (`sync`) or at once (`async`, the followers trailing by the shipping lag). A follower that is away holds no acknowledgement either way. |
...
| `CELASTRO_AUTO_FAILOVER` | `off` | `on` makes the steward promote the follower with the most recent copy once a holder has missed two sweeps, and makes a holder whose lease ran out refuse writes until it is renewed. Off, promotion is `PROMOTE SHARD` by hand and no lease gates a write. |
| `CELASTRO_LEASE_SECS` | `60` | How long a lease lasts; the steward renews every node's every quarter of it, on a thread of its own, so a sweep that stretches under load renews nothing late. |
