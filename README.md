...
| **a container** | `docker run ... ghcr.io/celastro/celastro:0.50.0 --dir /data serve --bind 0.0.0.0` with `CELASTRO_TOKEN` | a volume at `/data` | the published port, `CELASTRO_TOKEN` | `FROM scratch`, static binary, not root, handles SIGTERM; [docs/container.md](docs/container.md) |
...
```
docker run --rm ghcr.io/celastro/celastro:0.50.0 demo
docker run --rm --network host -v celastro-data:/data ghcr.io/celastro/celastro:0.50.0 --dir /data serve
```
