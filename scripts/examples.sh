...
sql "merge the shards back" 'shard 0 is \[, \).*2 row\(s\) of shard 1 rebuilt' "$D" "MERGE SHARDS 0 AND 1 OF notes"
sql "count after the merge" '4' "$D" "SELECT count(*) FROM notes"
sql "split at the median, no key given" "split at 'n3'" "$D" "SPLIT SHARD 0 OF notes"
